...
const DoctorAdmin = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const fetchedData = snapshot.docs
          ...
          .filter((user) => user.role === "doctor"); // Filter for doctors only
        setData(fetchedData);
      } catch (error) {
  ...
  return (
    <div>
      <h2 className="text-center text-3xl font-bold mb-4">Doctors Data</h2>
      <div className="w-fit">
        <ul className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {data.map((item) => (
            <li
              key={item.id}
